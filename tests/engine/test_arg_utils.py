...
def test_engine_args_uses_random_default_seed(monkeypatch):
    monkeypatch.setattr("vllm.config.model.secrets.randbelow", lambda _: 41)

    assert EngineArgs(model="test").seed == 42


def test_engine_args_preserves_explicit_zero_seed():
    assert EngineArgs(model="test", seed=0).seed == 0
