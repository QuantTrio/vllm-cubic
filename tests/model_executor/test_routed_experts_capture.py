...
def test_get_num_experts_per_tok_config_aliases(field: str, value: int):
    assert _get_num_experts_per_tok(SimpleNamespace(**{field: value})) == value


def test_routed_experts_manager_accepts_kimi_topk_alias(monkeypatch):
    import vllm.model_executor.layers.fused_moe.routed_experts_capturer as rec

    class DummyFullAttentionSpec:
        block_size = 4

    monkeypatch.setattr(rec, "FullAttentionSpec", DummyFullAttentionSpec)
    ...
    kv_cache_config = SimpleNamespace(
        num_blocks=2,
        kv_cache_groups=[SimpleNamespace(kv_cache_spec=DummyFullAttentionSpec())],
    )

    manager = RoutedExpertsManager(vllm_config, kv_cache_config)

    assert manager.routed_experts_by_slot.shape == (8, 93, 16)
    assert manager.routed_experts_by_slot.dtype == np.uint16
