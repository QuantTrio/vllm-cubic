...
def test_cubic_calibration_uses_bounded_representative_buckets():
    ...
    assert buckets == (1, 16, 64, 256, 512)


def test_cubic_calibration_keeps_the_largest_small_bucket():
    assert cubic_warmup._calibration_token_buckets(32, ()) == (1, 16, 32)
...
def test_cubic_moe_calibration_adds_only_the_largest_runtime_bucket():
    ...
    assert cubic_warmup._moe_calibration_token_buckets(256, base[:4]) == base[:4]
...
def test_cubic_resident_validation_uses_normalized_output_error() -> None:
...
def test_cubic_linear_residency_counts_every_repeated_layer(monkeypatch) -> None:
    ...
    monkeypatch.setattr(torch.accelerator, "get_memory_info", lambda *_: (3200, 8000))
    installed: list[int] = []
    monkeypatch.setattr(
        cubic_warmup,
