...
@pytest.mark.parametrize(
    ("input_size", "weight_group_size", "expected"),
    [
        (5120, 512, 32),
        (8704, 32, 32),
        (3072, 256, 32),
...
def test_dynamic_a8_group_follows_local_packing_geometry(
    input_size: int, weight_group_size: int, expected: int
) -> None:
    assert (
        cubic_dynamic_a8_group_size(
...
def test_runtime_memory_accounts_for_every_cubic_bit_width(num_bits: int) -> None:
    ...
    assert memory.carrier_replacement_extra_bytes == (257 - (257 * num_bits + 7) // 8)
