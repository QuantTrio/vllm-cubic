...
def test_installation_is_shape_specific_and_unquantized(
    ...
    monkeypatch.setattr(k3_gemm, "_is_sm90", lambda: False)
...
def test_sm90_table_is_decode_only() -> None:
    for spec in k3_gemm.KIMI_K3_SM90_PROJECTIONS.values():
        assert set(_num_tokens for _num_tokens, _ in spec.cute_configs) == {1}
        assert not spec.dsv3_tokens


def test_active_projection_table_prefers_current_architecture(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(k3_gemm, "_is_sm90", lambda: True)
    monkeypatch.setattr(k3_gemm, "_is_sm103", lambda: False)

    assert k3_gemm._active_projection_table() is k3_gemm.KIMI_K3_SM90_PROJECTIONS
...
def _require_sm90_and_cute() -> None:
    if not torch.cuda.is_available() or torch.cuda.get_device_capability() != (9, 0):
        pytest.skip("Kimi-K3 Hopper selection requires SM90")
    if not k3_gemm.shape_dynamic_skinny_gemm.is_available():
        pytest.skip("CuTe DSL is not available")
...
def test_sm90_selected_shapes(n: int, k: int) -> None:
    _require_sm90_and_cute()
...
def test_sm90_residual_epilogue() -> None:
    _require_sm90_and_cute()
    n, k = 7168, 3584
    torch.manual_seed(43)
    x = torch.randn(1, k, dtype=torch.bfloat16, device="cuda")
    weight = torch.randn(n, k, dtype=torch.bfloat16, device="cuda")
    residual = torch.randn(1, n, dtype=torch.bfloat16, device="cuda")

    output = k3_gemm.try_low_latency_gemm(x, weight, residual)

    assert output is not None
    reference = torch.addmm(residual, x, weight.t())
    cosine = torch.nn.functional.cosine_similarity(
        output.float().flatten(), reference.float().flatten(), dim=0
    ).item()
    assert cosine > 0.999
