...
def _assert_cubic8_close(
    cache: torch.Tensor, slots: torch.Tensor, expected: torch.Tensor
) -> None:
    from vllm.v1.attention.ops.cubic8_mla import dequantize_cubic8_mla_cache

    actual = _cache_rows(
        dequantize_cubic8_mla_cache(cache, expected.shape[-1], expected.dtype),
        slots,
    )
    error = (actual.float() - expected.float()).square().mean().sqrt()
    reference_rms = expected.float().square().mean().sqrt().clamp_min(1.0e-12)
    assert (error / reference_rms).item() < 0.01


@pytest.mark.parametrize(
    "cache_kind",
    ["bf16", "fp8_bf16_query", "fp8", "fp8_ds_mla", "cubic8"],
)
...
def test_prefill_epilogue_fuses_gptj_rope(cache_kind: str) -> None:
    ...
    if cache_kind == "bf16":
    ...
    elif cache_kind == "fp8_bf16_query":
        cache = torch.zeros(
        ...
        cache = torch.zeros(2, _BLOCK_SIZE, 648, device="cuda", dtype=torch.uint8)
        q_actual = q.clone()
        k_actual = fused_mla_key_concat_kv_cache_insert(
            q_actual,
            k_nope,
        ...
        torch.testing.assert_close(q_actual, q_expected)
        torch.testing.assert_close(k_actual, k_expected)
        _assert_cubic8_close(cache, slots, cache_expected)


@pytest.mark.parametrize(
    "cache_kind", ["bf16", "fp8_cache_only", "fp8", "fp8_ds_mla", "cubic8"]
)
@torch.inference_mode()
def test_decode_epilogue_fuses_gptj_rope(cache_kind: str) -> None:
    ...
    if cache_kind == "bf16":
        ...
        torch.testing.assert_close(q_actual, q_expected)
        torch.testing.assert_close(_cache_rows(cache, slots), cache_expected)
    elif cache_kind in ("fp8_cache_only", "fp8"):
        cache = torch.zeros(
            2, _BLOCK_SIZE, 576, device="cuda", dtype=torch.float8_e4m3fn
        )
        one = torch.ones(1, device="cuda", dtype=torch.float32)
        if cache_kind == "fp8_cache_only":
            q_actual = fused_mla_decode_q_concat_kv_cache_insert(
                ql_nope,
                q_pe,
                kv_c,
                ...
                slots,
                cache_only_fp8=True,
                bf16_cache_scale=one,
                **kwargs,
            )
            assert q_actual.dtype == _DTYPE
            torch.testing.assert_close(q_actual, q_expected)
        else:
            q_actual = fused_mla_decode_q_concat_kv_cache_insert(
                ql_nope,
