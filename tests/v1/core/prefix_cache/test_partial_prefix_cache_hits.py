...
def test_mamba_align_split_live_state_allows_latency_chunks():
    """A live request can stop within its current Mamba state block.

    Small chunks may advance the state in place, but a chunk may not cross a
    block boundary: the exact boundary state must remain cacheable before the
    following step migrates it to the next block.
    """
    block_size = 1472
    mock = SimpleNamespace(
        cache_config=SimpleNamespace(block_size=block_size),
    ...
    split = Scheduler._mamba_block_aligned_split
    req = make_request("live", [0] * 4096, 32, sha256)
    ...
    assert (
        split(
            ...
            request=req,
            num_new_tokens=128,
            allow_partial_live_state=True,
        )
        == 128
    )

    req.num_computed_tokens = 1408
    assert (
        split(
            self=mock,
            request=req,
    ...
    # The cache-oriented path retains its original aligned behavior.
    req.num_computed_tokens = 0
    assert split(self=mock, request=req, num_new_tokens=128) == 0
