...
@pytest.mark.parametrize(
    "max_num_mixed_prefill_tokens, expected_prefills",
    [(-1, 2), (0, 10)],
)
def test_mixed_prefill_budget_bounds_decode_head_of_line_blocking(
    ...
    (decode_req,) = create_requests(num_requests=1, num_tokens=10, req_ids=["decode"])
    ...
    prefills = create_requests(
        num_requests=10,
        num_tokens=100,
        req_ids=[f"prefill-{i}" for i in range(10)],
    )
    ...
    assert len(output.scheduled_new_reqs) == expected_prefills
    if max_num_mixed_prefill_tokens < 0:
        scheduled_prefill_tokens = sum(
            num_tokens
            for req_id, num_tokens in output.num_scheduled_tokens.items()
            if req_id != "decode"
        )
        assert scheduled_prefill_tokens <= scheduler.max_num_mixed_prefill_tokens


@pytest.mark.parametrize(
    ("max_num_batched_tokens", "expected_budget"),
    [(2048, 256), (1024, 256), (512, 128), (256, 64)],
)
def test_auto_mixed_prefill_budget_scales_for_small_devices(
    max_num_batched_tokens: int,
    expected_budget: int,
):
    scheduler = create_scheduler(
        max_num_batched_tokens=max_num_batched_tokens,
        max_num_mixed_prefill_tokens=-1,
    )
    assert scheduler.max_num_mixed_prefill_tokens == expected_budget
