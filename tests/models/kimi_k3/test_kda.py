...
def test_kda_state_shape_reserves_speculative_conv_history():
    conv_state_shape, temporal_state_shape = MambaStateShapeCalculator.kda_state_shape(
        tp_world_size=8,
        num_heads=96,
        head_dim=128,
        conv_kernel_size=4,
    ...
    assert sorted(conv_state_shape) == [10, 4608]
    assert temporal_state_shape == (12, 128, 128)
...
@torch.inference_mode()
def test_chunk_kda_live_prefill_split_matches_single_chunk(chunk_size: int):
    """Persisting final state makes latency-sized prefill chunks equivalent."""
    H, D, T = 8, 64, 512
    ...
    raw_beta = torch.randn(1, T, H, dtype=dtype, device=DEVICE)
    A_log = torch.randn(H, dtype=torch.float32, device=DEVICE) * 0.5
    dt_bias = torch.randn(H * D, dtype=torch.float32, device=DEVICE) * 0.1
    initial_state = torch.randn(1, H, D, D, dtype=torch.float32, device=DEVICE)

    full_out, full_state = chunk_kda_with_fused_gate(
        q=q.clone(),
        k=k.clone(),
        v=v.clone(),
        raw_g=raw_g.clone(),
    ...
    for start in range(0, T, chunk_size):
        ...
        out, state = chunk_kda_with_fused_gate(
            ...
            k=k[:, start:end].clone(),
            v=v[:, start:end].clone(),
            raw_g=raw_g[:, start:end].clone(),
            raw_beta=raw_beta[:, start:end].clone(),
            A_log=A_log,
            g_bias=dt_bias,
            initial_state=state,
            output_final_state=True,
    ...
    split_out = torch.cat(split_outputs, dim=1)
    assert full_state is not None
    assert_close("o", full_out, split_out, 2e-3, err_atol=2e-3)
    assert_close("ht", full_state, state, 2e-3, err_atol=2e-3)
