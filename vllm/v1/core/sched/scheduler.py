...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        configured_mixed_prefill_budget = (
            self.scheduler_config.max_num_mixed_prefill_tokens
        )
        if configured_mixed_prefill_budget < 0:
            # Keep mixed iterations latency-oriented by default. The fraction
            # scales with the deployment's normal batch budget, while the cap
            # prevents very large throughput-oriented batches from introducing
            # multi-second decode gaps.
            configured_mixed_prefill_budget = min(
                256, max(32, self.max_num_scheduled_tokens // 4)
            )
        self.max_num_mixed_prefill_tokens = configured_mixed_prefill_budget
    ...
    def _mamba_block_aligned_split(
        ...
        allow_partial_live_state: bool = False,
    ) -> int:
        ...
        # boundaries. When decode requests share the iteration, the live
        # request may stop inside its current block: Mamba kernels preserve the
        # running state in that block's state slot. Such a chunk must not cross
        # a block boundary, because the old block still needs the exact state
        # at its end for prefix-cache reuse. Clip at the boundary and let the
        # next iteration migrate that boundary state to the next block.
        if end < last_cache_position:
            if allow_partial_live_state:
                end = min(end, next_block_boundary)
            else:
                # May yield an empty chunk when the budget cannot reach the
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            if request.is_prefill_chunk and has_active_decode:
                remaining_mixed_prefill_budget = (
                    self.max_num_mixed_prefill_tokens - mixed_prefill_tokens
                )
                if self.max_num_mixed_prefill_tokens > 0:
                    if remaining_mixed_prefill_budget <= 0:
                        req_index += 1
                        continue
                    num_new_tokens = min(num_new_tokens, remaining_mixed_prefill_budget)
            ...
            if self.need_mamba_block_aligned_split:
                num_new_tokens = self._mamba_block_aligned_split(
                    request,
                    num_new_tokens,
                    allow_partial_live_state=has_active_decode,
                )
            ...
            if request.is_prefill_chunk:
                mixed_prefill_tokens += num_new_tokens
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                if load_kv_async:
                    ...
                    if has_active_decode and self.max_num_mixed_prefill_tokens > 0:
                        remaining_mixed_prefill_budget = (
                            self.max_num_mixed_prefill_tokens - mixed_prefill_tokens
                        )
                        if remaining_mixed_prefill_budget <= 0:
                            break
                        # Admission control is preferable to splitting a short
                        # request solely to fill the remainder: it preserves
                        # cache alignment and avoids turning one burst into a
                        # nearly full mixed iteration. If the oldest request is
                        # itself larger than the budget, admit one bounded chunk
                        # so continuous decode traffic cannot starve prefills.
                        if num_new_tokens > remaining_mixed_prefill_budget:
                            if mixed_prefill_tokens > 0:
                                break
                            num_new_tokens = min(
                                num_new_tokens, remaining_mixed_prefill_budget
                            )
