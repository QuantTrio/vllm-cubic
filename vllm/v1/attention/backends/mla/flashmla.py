...
# FlashMLA's native dense FP8 entry point couples an FP8 cache to an FP8
# query.  For cache-only FP8 we keep the query in the model dtype and use the
# existing Triton MLA reader, which dequantizes cache values at load time.
_MIN_WORK_PER_FP8_Q16_SPLIT = 512
_FP8_Q16_SPLIT_OCCUPANCY_MULTIPLIER = 2
# SM90 measurements show the native producer/consumer pipeline wins while one
# decode step covers at most ~32K cached tokens. Above that point the Triton
# reader scales better across requests and long sequences.
_FP8_Q16_NATIVE_MAX_TOKEN_WORK = 32 * 1024


def _compute_fp8_q16_num_kv_splits(max_seq_len: int, sm_count: int) -> int:
    ideal = triton.next_power_of_2(max(1, max_seq_len // _MIN_WORK_PER_FP8_Q16_SPLIT))
    return min(ideal, sm_count * _FP8_Q16_SPLIT_OCCUPANCY_MULTIPLIER)
...
class FlashMLAMetadataBuilder(MLACommonMetadataBuilder[FlashMLAMetadata]):
    ...
    def get_cudagraph_support(cls, vllm_config, kv_cache_spec) -> AttentionCGSupport:
        # Cubic8 keeps its dynamic cache boundary outside full-model graphs.
        # fp8_q16 is graph-safe, but controlled H200 TP8+EP measurements of a
        # hybrid linear/full-attention MoE show that FULL decode graphs regress
        # from 55.4-55.8 to ~53 tok/s
        # (and B2 from 98.6-99.0 to ~94.5). Keep both query-protected cache
        # formats on piecewise graphs while retaining CUDA graphs around the
        # attention boundary. Standard FP8/BF16 behavior is unchanged.
    ...
    def determine_prefill_query_data_type(
        ...
        """Keep prefill Q/K/V in model dtype for a cache-only FP8 test.

        The native FlashMLA FP8 decode entry point quantizes Q as part of its
        API contract.  That couples query error to cache error and makes an
        FP8-KV quality comparison ambiguous.  Prefill already has a model-dtype
        path, so select it unconditionally for plain FP8 caches.
        """
        ...
        if cache_dtype in (
            ...
            logger.info_once(
                "%s KV cache-only mode (prefill): current Q/K/V attention "
                "operands use %s.",
                cache_dtype,
                model_dtype,
            )
            return model_dtype
        return MLACommonMetadataBuilder.determine_prefill_query_data_type(
            vllm_config, model_dtype, cache_dtype
        )
    ...
    def __init__(
        ...
        if self.fp8_cache_only or self.cubic8_cache:
            # The BF16-Q Triton reader is a single-token decode kernel.  Route
            # short prompts through the regular prefill path instead of the
            # FlashMLA uniform multi-token decode path.
        ...
        # Plain FP8 uses FlashMLA's FP8-Q path. Query-protected FP8 uses the
        # same scheduler with a distinct BF16-Q / FP8-KV SM90 kernel.
        ...
        if (
            ...
            max_splits = _compute_fp8_q16_num_kv_splits(
            ...
            if self.cubic8_cache:
                ...
                workspace_specs.extend(
                    [
                        ...
                        (
                            (max_materialize_tokens // page_size,),
                            torch.int32,
                        ),
                    ]
                )
    ...
    def _build_decode(
        ...
        if getattr(self, "use_native_compressed_scheduler", self.is_fp8_kvcache):
...
class FlashMLAImpl(MLACommonImpl[FlashMLAMetadata]):
    ...
    def __init__(
        ...
        **mla_args,
        ...
        if self._fp8_cache_only or self._cubic8_cache:
            # Do not let the common MLA layer quantize the decode query.  The
            # FP8 cache is dequantized on load by _forward_fp8_q16.
            self.supports_quant_query_input = False
            q_device_index = torch.accelerator.current_device_index()
            self._sm_count = num_compute_units(q_device_index)
            logger.info_once(
                "%s MLA cache-only decode: query uses the model dtype and "
                "cached latent values are dequantized inside the attention "
                "kernel on GPU %d.",
                self.kv_cache_dtype,
                q_device_index,
            )
    ...
    def _forward_fp8_q16(
        ...
        """BF16/FP16 query against a plain per-tensor FP8 MLA cache."""
        assert q.ndim == 3, (
            "FP8 cache-only FlashMLA mode supports single-token decode; "
            f"received query shape {tuple(q.shape)}"
        )
        assert attn_metadata.decode is not None

        if (
            getattr(self, "_fp8_cache_only", False)
            ...
            native_output, native_lse = flash_mla_with_kvcache_fp8_q16(
                q=q.unsqueeze(1),
            ...
            return native_output.squeeze(1), native_lse.squeeze(-1)

        if (
            getattr(self, "_native_cubic8_cache", False)
            and q.dtype == torch.bfloat16
            and q.shape[-1] == 576
            ...
            assert scheduler.tile_scheduler_metadata is not None
            assert scheduler.num_splits is not None
            native_output, native_lse = flash_mla_with_kvcache_cubic8(
                q=q.unsqueeze(1),
                k_cache=kv_c_and_k_pe_cache.unsqueeze(2),
                block_table=attn_metadata.decode.block_table,
                cache_seqlens=attn_metadata.decode.seq_lens,
        ...
        if envs.VLLM_BATCH_INVARIANT:
            ...
            num_kv_splits = _compute_fp8_q16_num_kv_splits(
    ...
    def _forward_fp8_q16_prefill_context(
        ...
        """Attend BF16 absorbed-Q directly against paged FP8 MLA context.

        Each request is submitted as one variable-length query to the native
        SM90 compressed-cache kernel.  Unlike the generic context path this
        never gathers the latent cache into a BF16 workspace and never expands
        cached K/V before attention.
        """
        assert self._fp8_cache_only
        assert q.dtype == torch.bfloat16
        assert prefill_metadata.query_lens is not None
        assert prefill_metadata.context_lens is not None
        assert prefill_metadata.context_lens_cpu is not None
        ...
        query_offset = 0
        for request_idx, query_len in enumerate(prefill_metadata.query_lens):
            query_end = query_offset + query_len
            request_q = q[query_offset:query_end]
            request_context_len = prefill_metadata.context_lens[
                request_idx : request_idx + 1
            ]

            # The chunked-context path is entered only when at least one request
            # has context. Requests without context still need neutral values so
            # merge_attn_states selects their suffix result exactly.
            if prefill_metadata.context_lens_cpu[request_idx] == 0:
                outputs.append(
                    torch.zeros(
                        query_len,
                        q.shape[1],
            ...
            # FlashMLA returns LSE as [batch, heads, query].
