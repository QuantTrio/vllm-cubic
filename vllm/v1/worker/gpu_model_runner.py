...
class GPUModelRunner(
    ...
    def profile_compile_run(self) -> None:
        """Compile the language-model profile shape without profiling state.

        This deliberately omits the multimodal encoder, sampler, and EPLB
        bookkeeping from :meth:`profile_run`.  In particular, repeating the
        full distributed profile run is not generally safe, while this single
        eager dummy forward is sufficient to move one-shot torch.compile and
        Triton allocations out of the subsequent memory measurement.
        """
        hidden_states, last_hidden_states = self._dummy_run(
            self.max_num_tokens,
        ...
        self._sync_device()
        del hidden_states, last_hidden_states
        gc.collect()
