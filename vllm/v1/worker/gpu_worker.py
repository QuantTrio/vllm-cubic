...
class Worker(WorkerBase):
    ...
    def load_model(self, *, load_dummy_weights: bool = False) -> None:
        ...
        # Cubic calibrates both activation modes and production prefill shapes.
        # Run immediately after weight loading, before memory profiling sizes
        # and allocates KV cache from the remaining device memory. The tuner
        # releases all synthetic tensors before returning, and its selected
        # tactics are therefore ready for both profile runs and graph capture.
    ...
    def determine_available_memory(self) -> int:
        ...
        # Compile/JIT the exact profiling shape before measuring memory. A cold
        # torch.compile/Triton cache can otherwise make the first boot reserve
        # materially less KV memory than an identical warm-cache boot by
        # counting one-shot compiler temporaries as steady activation memory.
        self.model_runner.profile_compile_run()
        gc.collect()
        torch.accelerator.empty_cache()
        torch.accelerator.synchronize()

        # Measure the complete profile after one-shot compilation is gone.
