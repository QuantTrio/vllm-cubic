...
class KVQuantMode(IntEnum):
    ...
    CUBIC8_GROUPWISE = 6  # signed codes + FP32 scale + fixed-curve id per group
...
def cubic8_mla_token_size_bytes(head_size: int) -> int:
    ...
    # Metadata is struct-of-arrays: one aligned FP32 scale per group followed
    # by one fixed-curve id byte per group. The complete metadata tail is
    # aligned so consecutive token records remain vector-load friendly.
...
class MLAAttentionSpec(FullAttentionSpec):
    ...
    def merge(cls, specs: list[Self]) -> Self:
        ...
        assert (
            ...
            "cache dtype, quantization mode, alignment, compress ratio, model "
            "version, and KV block stride indexing."
        )
        return cls(
            block_size=specs[0].block_size,
            num_kv_heads=specs[0].num_kv_heads,
...
class SlidingWindowMLASpec(SlidingWindowSpec):
    ...
    def merge(cls, specs: list[Self]) -> Self:
        ...
        assert (
            ...
            "All attention layers in the same KV cache group must use the same "
            "cache dtype, quantization mode, alignment, compress ratio, model "
            "version, sliding window size, and KV block stride indexing."
        )
        return cls(
            block_size=specs[0].block_size,
            num_kv_heads=specs[0].num_kv_heads,
