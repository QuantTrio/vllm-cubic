...
class CompletionRequest(OpenAIBaseModel):
    ...
    def to_sampling_params(
        ...
        if default_stop:
            request_stops = [stop] if isinstance(stop, str) else list(stop or [])
            default_stops = (
                [default_stop] if isinstance(default_stop, str) else list(default_stop)
            )
            stop = list(dict.fromkeys([*request_stops, *default_stops]))
