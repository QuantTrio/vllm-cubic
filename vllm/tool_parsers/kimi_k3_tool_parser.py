...
class KimiK3ToolParser(ToolParser):
    ...
    def __init__(self, tokenizer: TokenizerLike, tools: list[Tool] | None = None):
        ...
        self.think_close = "<|close|>think<|sep|>"
        ...
        self._think_close_re = re.compile(_C + r"\s*think\s*" + _S)
    ...
    def _extract_response_content(self, current_text: str) -> str | None:
        ...
        body_start = max(
            m_open.end() if m_open is not None else 0,
            m_think_close.end() if m_think_close is not None else 0,
        )
        ...
        if candidates:
            ...
            think_overlap = _partial_tag_overlap(
                current_text,
                self.think_close,
            )
            overlap = max(
            ...
            sendable_idx = 0 if think_overlap else len(current_text) - overlap
