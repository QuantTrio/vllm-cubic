...
"""Kimi-K3 decode GEMM selection for unquantized BF16 on SM90 and SM103.
...
# H200/SM90 decode-only measurements. Keep only the routed projections that
# preserve the Kimi-K3 recitation evaluation when enabled together.
KIMI_K3_SM90_PROJECTIONS: dict[tuple[int, int], ProjectionSpec] = {
    (3584, 7168): ProjectionSpec(
        3584,
        7168,
        cute_configs=((1, _cute(1, 224, 2, 2)),),
        name="routed_expert_down_proj",
    ),
    (7168, 3584): ProjectionSpec(
        7168,
        3584,
        cute_configs=((1, _cute(1, 224, 2, 2)),),
        residual_configs=((1, _cute(1, 224, 2, 2)),),
        name="routed_expert_up_proj",
    ),
}
...
def _active_projection_table() -> dict[tuple[int, int], ProjectionSpec] | None:
    if _is_sm90():
        return KIMI_K3_SM90_PROJECTIONS
    if _is_sm103():
...
def try_low_latency_gemm(
    ...
    use this path.
    """
    table = _active_projection_table()
    if envs.VLLM_BATCH_INVARIANT or table is None or not _runtime_ok(x, weight):
        return None
...
def _compiled_low_latency_gemm(
    x: torch.Tensor,
    weight: torch.Tensor,
) -> torch.Tensor:
    output = try_low_latency_gemm(x, weight)
    if output is not None:
        return output
    return torch.nn.functional.linear(x, weight)


def _compiled_low_latency_gemm_fake(
    x: torch.Tensor,
    weight: torch.Tensor,
) -> torch.Tensor:
...
def _compiled_low_latency_gemm_residual(
    x: torch.Tensor,
    weight: torch.Tensor,
    residual: torch.Tensor,
) -> torch.Tensor:
    output = try_low_latency_gemm(x, weight, residual)
    if output is not None:
        return output
    return torch.addmm(residual, x, weight.t())


def _compiled_low_latency_gemm_residual_fake(
    x: torch.Tensor,
    weight: torch.Tensor,
    residual: torch.Tensor,
) -> torch.Tensor:
...
direct_register_custom_op(
    op_name="kimi_k3_low_latency_gemm",
    op_func=_compiled_low_latency_gemm,
    fake_impl=_compiled_low_latency_gemm_fake,
)
direct_register_custom_op(
    op_name="kimi_k3_low_latency_gemm_residual",
    op_func=_compiled_low_latency_gemm_residual,
    fake_impl=_compiled_low_latency_gemm_residual_fake,
)
    ...
    def apply(
        ...
        if (
            ...
            if torch.compiler.is_compiling():
                return torch.ops.vllm.kimi_k3_low_latency_gemm(x, layer.weight)
...
class KimiK3LowLatencyLinearMethod(_KimiK3LowLatencyApply, UnquantizedLinearMethod):
    ...
    def apply_with_residual(
        ...
        if (
            ...
            if torch.compiler.is_compiling():
                return torch.ops.vllm.kimi_k3_low_latency_gemm_residual(
...
def enable_kimi_k3_low_latency_gemm(
    ...
    """
    table = _active_projection_table()
    if dtype != torch.bfloat16 or table is None:
        return
