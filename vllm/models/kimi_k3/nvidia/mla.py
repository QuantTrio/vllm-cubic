...
def _kimi_k3_mla_attention(
    ...
    assert isinstance(layer, MultiHeadLatentAttention)
    layer._attention(positions, q, kv_c_normed, k_pe, attn_out)


def _kimi_k3_mla_attention_fake(
    positions: torch.Tensor,
    q: torch.Tensor,
    kv_c_normed: torch.Tensor,
    k_pe: torch.Tensor,
...
direct_register_custom_op(
    op_name="kimi_k3_mla_attention",
    op_func=_kimi_k3_mla_attention,
    mutates_args=["attn_out"],
    fake_impl=_kimi_k3_mla_attention_fake,
)
...
class MultiHeadLatentAttention(nn.Module, AttentionLayerBase):
    ...
    def __init__(
        ...
        self.impl.total_cp_world_size = 1
        self.impl.total_cp_rank = 0
        ...
        if (
            ...
            and "vllm::kimi_k3_mla_attention" not in splitting_ops
        ):
            splitting_ops.append("vllm::kimi_k3_mla_attention")
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig) -> KVCacheSpec:
        ...
        if self.kv_cache_dtype == "auto" and shared_page is not None:
            ...
            if block_size <= 0 or block_size * one_token > shared_page:
                raise ValueError(
                    "Unable to fit the unquantized Kimi-K3 MLA skip layer "
                    f"into the shared KV page ({shared_page} bytes)."
                )
        ...
        return MLAAttentionSpec(
            ...
            cache_dtype_str=self.kv_cache_dtype,
            kv_quant_mode=get_kv_quant_mode(self.kv_cache_dtype),
            non_causal=self.non_causal_multi_token_decode,
    ...
    def _forward_attn(
        ...
        if torch.compiler.is_compiling():
            torch.ops.vllm.kimi_k3_mla_attention(
                positions,
                q,
                kv_c_normed,
                k_pe,
    ...
    def _decode_concat_cache(
        ...
        """Fused decode query-concat + latent cache insert by cache dtype.

        Backends may opt out of quantized query input; in that cache-only FP8
        mode the query remains in the model dtype.
        """
        ...
        if is_quantized_kv_cache(self.kv_cache_dtype):
            if not self.impl.supports_quant_query_input:  # type: ignore[attr-defined]
                # Cache-only FP8 mode: preserve the BF16/FP16 query and only
                # quantize [kv_c_normed | k_pe] while writing the paged cache.
                cache = self.kv_cache
                ...
                return fused_mla_decode_q_concat_kv_cache_insert(
                    ...
                    slot_mapping,
                    cache_only_fp8=True,
                    bf16_cache_scale=self._k_scale,
                    positions=positions,
                    cos_sin_cache=cos_sin_cache,
                )
        ...
        return fused_mla_decode_q_concat_kv_cache_insert(
            ...
            self.kv_cache,
            slot_mapping,
            bf16_cache_scale=self._one_scale,
            positions=positions,
            cos_sin_cache=cos_sin_cache,
        )
    ...
    def _forward_prefill_fused(
        ...
        Supported configs (K3 fp8 policy):
          ...
          - plain fp8 cache   -> bf16 prefill q/k/v; only cache writes are fp8
        ...
        if self.kv_cache_dtype == "fp8_ds_mla":
            ...
            if fp8_prefill:
                # Query-quantizing prefill backends consume fp8 q/k/v directly.
                kv_cache = self.kv_cache
                ...
                # Hopper prefill backends use bf16 q/k/v, but the latent cache
                # is still stored as fp8.  This is the normal vLLM fallback
                # when prefill query quantization is unavailable.
        ...
        supports_out = getattr(prefill.prefill_backend, "supports_out", None)
        writes_out = not has_context and supports_out is not None and supports_out()
