...
def attn_res(
    ...
    if torch.compiler.is_compiling():
        return torch.ops.vllm.kimi_k3_attn_res_compile_safe(
            prefix,
            delta,
            blocks,
            norm_weight,
    ...
    if (not torch.compiler.is_compiling() and num_tokens >= 256) or num_blocks <= 1:
...
direct_register_custom_op(
    op_name="kimi_k3_attn_res_compile_safe",
    op_func=attn_res,
    mutates_args=["prefix", "blocks"],
