...
def _kimi_k3_kda_attention(
    ...
    assert isinstance(layer, KimiK3DeltaAttention)
    layer._forward(mixed_qkv, g1, g2, beta, core_attn_out)


def _kimi_k3_kda_attention_fake(
    mixed_qkv: torch.Tensor,
    g1: torch.Tensor,
    g2: torch.Tensor,
    beta: torch.Tensor,
...
direct_register_custom_op(
    ...
    op_func=_kimi_k3_kda_attention,
    mutates_args=["core_attn_out"],
    fake_impl=_kimi_k3_kda_attention_fake,
)
...
def is_flashkda_supported(
    ...
    return (
        ...
        and importlib.util.find_spec("vllm._flashkda_C") is not None
    )
...
class KimiK3DeltaAttention(GatedDeltaNetAttention):
    ...
    def __init__(
        ...
        if (
            ...
            and "vllm::kimi_k3_kda_attention" not in splitting_ops
        ):
            splitting_ops.append("vllm::kimi_k3_kda_attention")
