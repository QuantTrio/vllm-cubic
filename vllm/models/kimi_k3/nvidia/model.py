...
def _attn_res_linear_weight(layer: ReplicatedLinear) -> torch.Tensor:
    weight = getattr(layer, "weight", None)
    if weight is None:
        dequantize = getattr(layer.quant_method, "dequantize", None)
        if dequantize is None:
            raise TypeError(
                f"{type(layer.quant_method).__name__} cannot provide an AttnRes weight."
            )
        weight = dequantize(layer)
...
def match_kimi_k3_expert_load_targets(
    weight_name: str,
    mapping: list[tuple[str, str, int, str]],
) -> list[tuple[str, int, str]]:
    return [
        (
            weight_name.replace(checkpoint_name, param_name),
            expert_id,
            shard_id,
        )
        for param_name, checkpoint_name, expert_id, shard_id in mapping
        if checkpoint_name in weight_name
    ]


def select_kimi_k3_cubic_layer_mapping(
    weight_name: str,
    mappings: dict[int, list[tuple[str, str, int, str]]],
) -> list[tuple[str, str, int, str]] | None:
    return next(
        (
            mapping
            for layer_idx, mapping in mappings.items()
...
class KimiDecoderLayer(nn.Module):
    def __init__(
        ...
        if self.use_attn_res:
            ...
            mlp_res_prefix = f"{prefix}.mlp_res_proj"
            has_explicit_scheme = getattr(quant_config, "has_explicit_scheme", None)
            mlp_res_quant_config = (
                quant_config
                if (
                    quant_config is not None
                    and quant_config.get_name() == "cubic"
                    and has_explicit_scheme is not None
                    and has_explicit_scheme(mlp_res_prefix)
                )
                else None
            )
            self.mlp_res_proj = ReplicatedLinear(
                ...
                1,
                bias=False,
                quant_config=mlp_res_quant_config,
                prefix=mlp_res_prefix,
            )
    ...
    def _post_attn_norm(
        ...
        hidden_states = attn_res(
            ...
            _attn_res_linear_weight(self.mlp_res_proj),
...
class KimiLinearModel(nn.Module, EagleModelMixin):
    ...
    def load_weights(
        ...
        cubic_expert_params_mappings: dict[int, list[tuple[str, str, int, str]]] = {}
        for module in self.modules():
            if not isinstance(module, RoutedExperts) or not hasattr(
                module, "w13_weight_packed"
            ):
                continue
            layer_parts = module.layer_name.split(".layers.", 1)
            if len(layer_parts) != 2:
                continue
            layer_idx = int(layer_parts[1].split(".", 1)[0])
            cubic_expert_params_mappings[layer_idx] = (
        ...
        for args in weights:
            ...
            if "rotary_emb.cos_cached" in name or "rotary_emb.sin_cached" in name:
                ...
                continue
            cubic_mapping = select_kimi_k3_cubic_layer_mapping(
                name,
                cubic_expert_params_mappings,
            )
            loaded_cubic_expert = False
            if cubic_mapping is not None:
                for (
                    mapped_name,
                    expert_id,
                    expert_shard_id,
                ) in match_kimi_k3_expert_load_targets(name, cubic_mapping):
                    ...
                    loaded_cubic_expert = True
            if loaded_cubic_expert:
                continue
...
class KimiK3ForConditionalGeneration(
    ...
    def __init__(
        ...
        if not self.language_model_only:
            self._init_vision_model(vllm_config, prefix, quant_config)
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]):
        skip_prefixes = (
            ["vision_tower.", "mm_projector."] if self.language_model_only else None
        )
