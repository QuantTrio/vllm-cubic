...
class SchedulerConfig:
    ...
    max_num_mixed_prefill_tokens: int = Field(default=0, ge=-1)
    """Maximum aggregate number of prefill tokens admitted in an iteration
    that also contains decode work. ``-1`` selects an automatic latency-oriented
    budget derived from ``max_num_batched_tokens``; ``0`` disables this limit
    (the default for non-Cubic models).

    This bounds head-of-line blocking for active decodes when multiple new
    requests arrive together, while leaving pure-prefill iterations unchanged.
    """
