        ...
        if self.method in ("ngram", "ngram_gpu"):
            ...
            if self.model is not None:
                ...
                # deepseek-ai/dspark_qwen3_8b_block7
                    ...
                    or "K3DSparkModel" in self.draft_model_config.architectures
                ):
                    self.method = "dspark"
                ...
                if self.method == "dspark" and (
                    ...
                    hf = self.draft_model_config.hf_config
                    if getattr(hf, "dspark_target_layer_ids", None) is None:
                        hf.dspark_target_layer_ids = hf.target_layer_ids
                    if getattr(hf, "n_predict", None) is None:
                        hf.n_predict = 7
                    if getattr(hf, "dspark_block_size", None) is None:
                        hf.dspark_block_size = hf.n_predict
