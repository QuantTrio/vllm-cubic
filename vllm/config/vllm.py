...
class VllmConfig:
    ...
    def __post_init__(self):
        ...
        # DBO must execute all-reduce at the ubatch runtime boundary so its
        # compute/communication stream hand-off is captured by the outer full
        # graph.  If all-reduce is left inside a pre-captured piecewise graph,
        # that graph permanently binds the collective to the compute stream
        # before any ubatch context exists.
        splitting_ops = self.compilation_config.splitting_ops
        if (
            self.parallel_config.use_ubatching
            and splitting_ops
            and "vllm::all_reduce" not in splitting_ops
        ):
            splitting_ops.append("vllm::all_reduce")
        ...
        if self.parallel_config.use_ubatching:
            ...
            supported_a2a_backend = a2a_backend in [
                "deepep_low_latency",
                "deepep_high_throughput",
                "nixl_ep",
            ]
            # With one DP rank, static TP+EP does not dispatch tokens through
            # an all-to-all manager. Its cross-rank synchronization is TP
            # all-reduce, which can use the generic DBO stream hand-off.
            static_ep_without_a2a = (
                self.parallel_config.data_parallel_size == 1
                and a2a_backend == "allgather_reducescatter"
            )
            assert supported_a2a_backend or static_ep_without_a2a, (
