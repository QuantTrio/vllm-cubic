...
class KimiK3ReasoningParser(ReasoningParser):
    ...
    def extract_reasoning(
        ...
        # The generation prefix consumes the opening marker, so a truncated
        # thinking response can contain neither marker. In thinking mode this
        # is still reasoning; instruct mode has already returned above.
    ...
    def _content_ready_to_emit(self, text: str) -> str:
        ...
        """
        # A token-ID transition can precede detokenization of the complete
        # think-close marker. Discard that late boundary and any punctuation
        # still attached to the reasoning channel.
        m_think_close = self._think_close_re.search(text)
        if m_think_close is not None:
            text = text[m_think_close.end() :]
