...
if TYPE_CHECKING:
    ...
    VLLM_ROUTED_DOWN_PROJ_STREAM_TOKEN_THRESHOLD: int = 512
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # Keep Cubic weight-only A16 as the default. This opt-in enables runtime
    # per-token INT8 activation quantization for Cubic Linear and MoE layers.
    "VLLM_CUBIC_DYNAMIC_A8": lambda: bool(int(os.getenv("VLLM_CUBIC_DYNAMIC_A8", "0"))),
    # Calibrate device-specific Cubic tactics during vLLM's kernel warmup,
    # before CUDA graph capture. Disable only for startup diagnostics.
    "VLLM_CUBIC_AUTOTUNE": lambda: bool(int(os.getenv("VLLM_CUBIC_AUTOTUNE", "1"))),
    ...
    "VLLM_ROUTED_DOWN_PROJ_STREAM_TOKEN_THRESHOLD": lambda: int(
        os.getenv("VLLM_ROUTED_DOWN_PROJ_STREAM_TOKEN_THRESHOLD", "512")
    ),
