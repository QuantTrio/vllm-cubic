...
def _cubic_task_weight(task: CalibrationTask) -> int:
    ...
    _, _, _, hidden, intermediate, top_k, local_experts, _, *tail = task
    tokens = int(tail[-1])
    return (
        int(hidden)
        * int(intermediate)
...
def _calibration_token_buckets(
    ...
    targets = (1, 8, 16, 64, 256, max_tokens)
    available = tuple(
...
def _warmup_cubic_linear_families(
    ...
    for layer, method in layers.values():
        ...
        if not assigned_tokens:
            continue
        logger.info(
            "Calibrating Cubic Linear A8/A16: W%d G=%d N=%d K=%d M=%s",
            bits,
            group_size,
            n,
            k,
        ...
        for m in token_buckets:
            ...
            phase_index = 0
            phase_total = 4
            for dynamic_a8, coefficient_a, coefficient_b in (
                (False, a16_a, a16_b),
                (True, a8_a, a8_b),
            ):
                calibrate_cubic_linear_execution(
            ...
            for dynamic_a8, func, coefficient_a, coefficient_b in (
                (False, cubic_linear, a16_a, a16_b),
                (True, cubic_linear_dynamic_a8, a8_a, a8_b),
            ):
                output = func(
                    x,
                    layer.weight_packed,
...
def _warmup_cubic_moe_families(
    ...
    for layer, method in layers.values():
        ...
        if not assigned_tokens:
            continue
        logger.info(
            "Calibrating Cubic MoE A8/A16: W%d G=%d H=%d I=%d top_k=%d experts=%d M=%s",
            bits,
            group_size,
            hidden,
            intermediate,
        ...
        for tokens in token_buckets:
            ...
            # grouping + four route-CTA projections + two optional backend
            # comparisons + A16/A8 execution selection + A16/A8 correctness.
            phase_total = 8 + int(can_calibrate_grouping) + 2 * int(calibrate_backend)
            if can_calibrate_grouping:
                phase_index += 1
                if progress is not None:
            ...
            for dynamic_a8, coefficients in (
                (False, (a16_a, a16_b, a16_w2_a, a16_w2_b)),
                (True, (a8_a, a8_b, a8_w2_a, a8_w2_b)),
            ):
            ...
            phase_index += 1
            if progress is not None:
                progress.phase(task, phase_index, phase_total, "A16 execution")
            calibrate_cubic_moe_execution(
                x,
                layer.w13_weight_packed,
                layer.w2_weight_packed,
            ...
            phase_index += 1
            if progress is not None:
                progress.phase(task, phase_index, phase_total, "A8 execution")
            a16_output = cubic_fused_moe(
                x,
                layer.w13_weight_packed,
                layer.w2_weight_packed,
                layer.w13_weight_scale,
            ...
            phase_index += 1
            if progress is not None:
                progress.phase(task, phase_index, phase_total, "A16 correctness")
            a8_output = cubic_fused_moe_dynamic_a8(
                x,
                layer.w13_weight_packed,
                layer.w2_weight_packed,
                layer.w13_weight_scale,
            ...
            phase_index += 1
            if progress is not None:
                progress.phase(task, phase_index, phase_total, "A8 correctness")
            if (
                not torch.isfinite(a16_output).all()
                or not torch.isfinite(a8_output).all()
            ):
                raise AssertionError(
