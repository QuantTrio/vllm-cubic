...
class MLAAttention(nn.Module, AttentionLayerBase):
    ...
    def forward_impl(
        ...
        if self.impl.dcp_world_size == -1:
            self.impl.dcp_world_size = max(get_dcp_group().world_size, 1)
    ...
    def process_weights_after_loading(self, act_dtype: torch.dtype):
        ...
        if hasattr(self, "W_UK_T") and hasattr(self, "W_UV"):
            cast(Any, self.impl).set_absorbed_prefill_weights(self.W_UK_T, self.W_UV)
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig) -> KVCacheSpec:
        ...
        shared_page = getattr(vllm_config.cache_config, "skip_page_size_padded", None)
        if self.kv_cache_dtype == "auto" and shared_page is not None:
            one_token = MLAAttentionSpec(
                block_size=1,
                num_kv_heads=1,
                head_size=self.head_size,
            ...
            block_size = shared_page // one_token
            if block_size <= 0 or block_size * one_token > shared_page:
                raise ValueError(
                    "Unable to fit an unquantized MLA skip layer into the "
                    f"shared KV page ({shared_page} bytes)."
                )
...
class MLACommonPrefillMetadata:
    ...
    max_query_len: int
    # CPU-derived lengths are retained so compressed-cache prefill kernels can
    # dispatch one variable-length query per request without synchronizing the
    # CUDA stream merely to recover metadata that the builder already had.
...
class MLACommonMetadataBuilder(AttentionMetadataBuilder[M]):
    ...
    def determine_prefill_query_data_type(
        ...
        if cache_dtype == "cubic8":
            logger.info_once(
                "%s MLA KV cache keeps prefill and decode queries in %s; "
                "only paged cache storage is quantized.",
                cache_dtype,
                model_dtype,
            )
            return model_dtype
        quantized_cache = is_quantized_kv_cache(cache_dtype)
        backend_supports_fp8_query = backend_supports_prefill_query_quantization()
        use_fp8 = (
            quantized_cache
            and vllm_config.attention_config.use_prefill_query_quantization
            and backend_supports_fp8_query
        )
        ...
        if use_fp8:
            ...
            logger.info_once(
                "FP8 KV cache is enabled, but the selected device/backend does "
                "not support FP8 prefill queries; using model-dtype prefill "
                "queries and quantizing only KV cache writes.",
            )
            return model_dtype
        elif quantized_cache and backend_supports_fp8_query:
            logger.warning_once(
    ...
    def __init__(
        ...
        self.kv_cache_spec = kv_cache_spec
        cache_dtype = getattr(self.kv_cache_spec, "cache_dtype_str", None)
        self.q_data_type = self.determine_prefill_query_data_type(
            vllm_config, self.model_config.dtype, cache_dtype
        )
    ...
    def build(
        ...
        if num_prefills > 0:
            ...
            prefill_metadata = MLACommonPrefillMetadata(
                ...
                query_lens=prefill_query_lens_cpu.tolist(),
                context_lens=context_lens_cpu.to(device, non_blocking=True),
                context_lens_cpu=context_lens_cpu.tolist(),
...
class MLACommonBaseImpl(MLAAttentionImpl[A], Generic[A]):
    ...
    def __init__(
        ...
        self.v_head_dim = v_head_dim
        self.kv_b_proj = kv_b_proj
        self._absorbed_W_UK_T: torch.Tensor | None = None
        self._absorbed_W_UV: torch.Tensor | None = None

    def set_absorbed_prefill_weights(
        self, W_UK_T: torch.Tensor, W_UV: torch.Tensor
    ) -> None:
        """Expose the standard MLA absorbed weights to compressed-cache prefill.

        Decode already uses these tensors to avoid materializing expanded K/V.
        Keeping the references on the backend lets a cache reader apply the
        same identity to chunked context without owning another weight copy.
        """
        self._absorbed_W_UK_T = W_UK_T
        self._absorbed_W_UV = W_UV
    ...
    def _compute_prefill_context(
        ...
        compressed_context = getattr(self, "_forward_fp8_q16_prefill_context", None)
        if (
            self.kv_cache_dtype == "fp8_q16"
            and q.dtype == torch.bfloat16
            and compressed_context is not None
            and self._absorbed_W_UK_T is not None
            and self._absorbed_W_UV is not None
        ):
            q_nope, q_pe = q.split(
                [self.qk_nope_head_dim, self.qk_rope_head_dim], dim=-1
            )
            # Q_nope @ W_UK^T moves the K up-projection to the query side.
            q_latent = torch.bmm(
                q_nope.transpose(0, 1), self._absorbed_W_UK_T
            ).transpose(0, 1)
            q_absorbed = torch.cat((q_latent, q_pe), dim=-1)
            latent_output, output_lse = compressed_context(
                q_absorbed, kv_c_and_k_pe_cache, prefill_metadata, k_scale
            )
            output = torch.empty(
                (*latent_output.shape[:-1], self.v_head_dim),
        ...
        for i in range(iters):
            ...
            if self.kv_cache_dtype == "fp8_ds_mla":
                ...
                kernel_cache_dtype = (
                    "fp8" if self.kv_cache_dtype == "fp8_q16" else self.kv_cache_dtype
                )
                ops.gather_and_maybe_dequant_cache(
                    ...
                    token_to_seq=prefill_metadata.chunked_context.token_to_seq[i],
                    num_tokens=prefill_metadata.chunked_context.chunk_total_token[i],
                    kv_cache_dtype=kernel_cache_dtype,
                    scale=k_scale,
                    seq_starts=prefill_metadata.chunked_context.starts[i],
                )
            else:
