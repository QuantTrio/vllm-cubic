...
class CubicScheme:
    ...
    def __post_init__(self) -> None:
        ...
        if self.group_size <= 0:
            raise ValueError("Cubic group_size must be positive.")
        if self.group_size * self.num_bits % 8:
            raise ValueError(
                "Cubic group boundaries must be byte aligned: "
                "group_size * num_bits must be divisible by 8."
            )
    ...
    def effective_bits(self) -> float:
        return self.num_bits + 64 / self.group_size
...
def dequantize_cubic(
    ...
    *,
    ...
    if scale.shape[-1] != expected_groups:
        raise ValueError(
            f"Expected {expected_groups} Cubic groups, got {scale.shape[-1]}."
        )
    ...
    group_indices = torch.arange(num_values, device=packed.device) // group_size
    scale_values = scale[..., group_indices]
...
class CubicLinearMethod(LinearMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs: Any,
        ...
        scale = GroupQuantScaleParameter(
            data=torch.empty(
                output_size_per_partition, num_groups, dtype=torch.float32
            ),
            **metadata_args,
        )
        a = GroupQuantScaleParameter(
            data=torch.empty(
                output_size_per_partition, num_groups, dtype=torch.float16
            ),
            **metadata_args,
        )
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if layer.weight_scale.shape[-1] != expected_groups:
...
class CubicMoEMethod(FusedMoEMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs: Any,
        ...
        for prefix, output_size, input_size in (
            ...
            metadata_shape = (num_experts, output_size, input_size // group_size)
            register(
    ...
    def process_weights_after_loading(self, layer: RoutedExperts) -> None:
        ...
        if (
            ...
            or layer.w2_weight_scale.shape[-1]
