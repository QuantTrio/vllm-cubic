...
_CUBIC_LINEAR_RESIDENCY_FREE_MEMORY_DIVISOR = 4
    ...
    *,
    ...
    return CubicRuntimeMemory(
        ...
        compact_metadata_bytes=(
            2 * num_groups + 12 * logical_tensors + output_groups
        ),
        expanded_metadata_bytes=8 * num_groups,
        carrier_bytes=carrier_bytes,
        carrier_replacement_extra_bytes=max(
            0, carrier_bytes - packed_weight_bytes
        ),
    )
...
def select_cubic_runtime_candidate(
    ...
    *,
    ...
    near_fastest = tuple(
        ...
        <= fastest_latency * (1.0 + compact_preference_margin)
    )
    return min(
        near_fastest,
