...
def _cubic_linear_use_gemv(
    *,
    ...
    device = torch.accelerator.current_device_index()
    exact = (device, dynamic_a8, num_bits, n, k, group_size, m)
...
@triton.autotune(
    ...
    key=["M", "N", "K", "NUM_BITS", "GROUP_SIZE", "BLOCK_K", "USE_GROUP_LUT"],
    cache_results=True,
)
@triton.jit
def _cubic_linear_kernel(
    ...
    for k_block in range(0, tl.cdiv(K, BLOCK_K)):
        ...
        if USE_GROUP_LUT:
            ...
            metadata_ptrs = offs_n * stride_sm + group * stride_sg
            ...
            metadata_ptrs = offs_n[None, :] * stride_sm + groups * stride_sg
...
def cubic_linear(
    ...
    *,
    ...
    if use_gemv and num_bits in (1, 2, 4, 8) and group_size in (128, 256, 512):
        packed_words = packed.view(torch.int32)
        grid = lambda meta: (
...
def _cubic_moe_kernel(
    ...
    for k_block in range(0, tl.cdiv(K, BLOCK_K)):
        ...
        if USE_GROUP_LUT:
            ...
            groups = global_k[:, None] // GROUP_SIZE
            metadata_ptrs = (
                expert_id * stride_se + offs_n[None, :] * stride_sn + groups * stride_sg
            )
            metadata_mask = weight_mask & (groups < NUM_GROUPS)
            scale = tl.load(
                scale_ptr + metadata_ptrs,
...
def _cubic_moe_dynamic_a8_kernel(
    ...
    for k_block in range(0, tl.cdiv(K, BLOCK_K)):
        ...
        k_mask = global_k < K
        activation = tl.load(
            input_ptr + input_rows[:, None] * stride_im + global_k[None, :] * stride_ik,
            mask=token_mask[:, None] & k_mask[None, :],
            other=0,
        )
        bit_positions = global_k[:, None] * NUM_BITS
...
def _launch_cubic_moe_gemv(
    ...
    *,
    ...
    if num_bits == 2 and group_size in (128, 256, 512) and logical_k % group_size == 0:
        packed_words = packed.view(torch.int32)
        grid = lambda meta: (
    ...
    if num_bits == 3 and group_size in (128, 256) and logical_k % group_size == 0:
        grid = lambda meta: (
            triton.cdiv(packed.shape[1], meta["BLOCK_N"]),
            route_ctas,
        )
    ...
    block_k = min(group_size, 64 if num_bits >= 7 else 256)
    grid = lambda meta: (
        triton.cdiv(packed.shape[1], meta["BLOCK_N"]),
        route_ctas,
    )
...
def _launch_cubic_moe_gemm(
    ...
    *,
    ...
    block_m, group_m = dense_block_m, 8
    block_k = 64 if group_size % 64 == 0 else 32
    use_group_lut = group_size >= block_k and group_size % block_k == 0
    n = packed.shape[1]
...
def _launch_cubic_moe_dynamic_a8(
    ...
    *,
    ...
    if use_gemv:
        ...
        if num_bits == 1 and logical_k % group_size == 0:
            packed_words = packed.view(torch.int32)
            input_words = inputs_q.view(torch.int32)
            grid = lambda meta: (
                triton.cdiv(packed.shape[1], meta["BLOCK_N"]),
        ...
        if (
            ...
            return
        if 3 <= num_bits <= 8 and logical_k % group_size == 0 and logical_k % 4 == 0:
            input_words = inputs_q.view(torch.int32)
            grid = lambda meta: (
                triton.cdiv(packed.shape[1], meta["BLOCK_N"]),
                route_ctas,
...
def calibrate_cubic_a8_moe_grouping(
    ...
    *,
    ...
    """Measure singleton versus pair route grouping for a complete MoE layer."""
    key = (
        torch.accelerator.current_device_index(),
        num_bits,
        hidden_size,
    ...
    init_logger(__name__).info(
        "Cubic A8 route grouping: W%d H=%d I=%d G=%d M=%d grouped=%d (%.4f ms)",
        num_bits,
        hidden_size,
        intermediate_size,
        group_size,
...
def cubic_fused_moe(
    ...
    *,
    ...
    route_ctas = None
    if use_gemv:
        fallback_route_ctas = min(sorted_ids.numel(), 128)
        route_ctas = _cubic_moe_route_ctas(
            dynamic_a8=False,
            num_bits=num_bits,
            n=w1.shape[1],
...
def cubic_fused_moe_dynamic_a8(
    ...
    *,
    ...
    if num_bits == 1:
        ...
        route_limit = 128
    use_gemv = route_limit is None or num_tokens <= route_limit
    use_gemv = _cubic_moe_use_gemv(
        dynamic_a8=True,
        num_bits=num_bits,
        hidden_size=hidden_size,
    ...
    if (
        ...
        if (
            ...
            while token_start < num_tokens:
                ...
                try:
                    chunk_route_output = torch.empty(
                        chunk_tokens,
                        top_k,
                        hidden_size,
                        device=hidden_states.device,
