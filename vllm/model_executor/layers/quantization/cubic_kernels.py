...
_A8BackendKey = tuple[int, int, int, int, int, int, int, int, int]
...
def _cubic8_w2_block_n(
    ...
    exact = (device, n, k, group_out, group_size, local_experts, local_routes)
    ...
    compatible = [
        ...
        and candidate_n == n
        and candidate_k == k
        and candidate_group_out == group_out
...
def _cubic_w2_a8_situ_tactic(
    ...
    device = torch.accelerator.current_device_index()
    exact = (device, n, k, group_out, group_size, local_experts, route_ctas)
...
def _launch_cubic_w2_a8_situ_pair(
    ...
    *,
    ...
    output_groupwise_a8: bool,
    output_bf16: bool,
    num_warps: int,
) -> None:
    group_out, group_size = group_shape
    grid = (triton.cdiv(output.shape[-1], block_n), route_ctas)
    _cubic_moe_pair_situ_gemv_2bit_a8_dp4a_kernel[grid](
        ...
        OUTPUT_GROUPWISE_A8=output_groupwise_a8,
...
def calibrate_cubic_w2_a8_situ(
    *,
    ...
    if n % group_out or k % group_size or k % 4:
        return
    device = torch.accelerator.current_device_index()
    ...
    for route_ctas in route_ctas_values:
        ...
        for block_n, num_warps in _CUBIC_W2_A8_SITU_CANDIDATES:
            ...
            def launch(
                ...
                _launch_cubic_w2_a8_situ_pair(
                    ...
                    expert_ids,
                    count,
                    group_shape=(group_out, group_size),
        ...
        init_logger(__name__).info(
            "Cubic W2 A8 SITU tactic: N=%d K=%d G=%dx%d experts=%d/%d "
            "route_ctas=%d "
    ...
    for k_block in range(0, tl.cdiv(K, BLOCK_K)):
        ...
        if NUM_BITS == 1:
            ...
            if NUM_BITS == 2:
                ...
                weight = tl.where(signed < 0, -weight, weight)
        weight = (weight * weight_scale[:, None]).to(
            input_ptr.dtype.element_ty
        ).to(tl.float32)
...
def _cubic_load_compact_metadata(
    ...
    if EXPANDED_METADATA:
        scale = tl.load(
            scale_code_ptr + metadata_offsets, mask=mask, other=0.0
        ).to(tl.float32)
        cubic_a = tl.load(
            packed_ab_ptr + metadata_offsets, mask=mask, other=1.0
        ).to(tl.float32)
        cubic_b = tl.load(
            scale_global_ptr + metadata_offsets, mask=mask, other=0.0
        ).to(tl.float32)
        return scale, cubic_a, cubic_b
    global_index = tl.load(
        global_index_ptr + output_groups, mask=mask, other=0
    ).to(tl.int32)
    scale_global = tl.load(
        scale_global_ptr + global_index, mask=mask, other=0.0
    ).to(tl.float32)
    a_global = tl.load(
        a_global_ptr + global_index, mask=mask, other=0.0
    ).to(tl.float32)
    b_global = tl.load(
        b_global_ptr + global_index, mask=mask, other=0.0
    ).to(tl.float32)
    scale_code = tl.load(
        scale_code_ptr + metadata_offsets, mask=mask, other=0
    ).to(tl.float32)
    packed_ab = tl.load(
        packed_ab_ptr + metadata_offsets, mask=mask, other=0
    ).to(tl.int32)
...
def _cubic_linear_compact_w4_g32_gemv_kernel(
    ...
    for group in tl.range(0, NUM_GROUPS, num_stages=1):
        ...
        weight_low = (weight_low * scale[:, None]).to(
            input_ptr.dtype.element_ty
        ).to(tl.float32)
        weight_high = (weight_high * scale[:, None]).to(
            input_ptr.dtype.element_ty
        ).to(tl.float32)
...
def cubic_linear_compact(
    ...
    *,
    ...
    use_gemv = x_2d.shape[0] <= 8 and not envs.VLLM_BATCH_INVARIANT
    if use_gemv:
...
def _cubic_linear_dynamic_a8_compact_gemv_kernel(
    ...
    activation_scale = tl.load(
        input_scale_ptr + row, mask=row < M, other=0.0
    ).to(tl.float32)
    accumulator = tl.zeros((BLOCK_N,), dtype=tl.float32)
    for k_block in range(0, tl.cdiv(K, BLOCK_K)):
        global_k = k_block * BLOCK_K + offs_k
...
def _cubic_linear_dynamic_a8_compact_kernel(
    ...
    n_mask = offs_n < N
    activation_scale = tl.load(
        input_scale_ptr + offs_m, mask=offs_m < M, other=0.0
    ).to(tl.float32)[:, None]
    accumulator = tl.zeros((BLOCK_M, BLOCK_N), dtype=tl.float32)
    for k_block in range(0, tl.cdiv(K, BLOCK_K)):
        global_k = k_block * BLOCK_K + offs_k
...
def cubic_linear_dynamic_a8_compact(
    ...
    *,
    ...
    if x.shape[-1] != input_size:
        raise ValueError(
            f"Compact Cubic A8 expected input width {input_size}, "
            f"got {x.shape[-1]}."
        )
...
def cubic_linear_dynamic_a8_precomputed(
    ...
    *,
    ...
    if envs.VLLM_BATCH_INVARIANT:
        use_gemv = False
    block_k_fallback = min(group_size, 128)
    if envs.VLLM_BATCH_INVARIANT:
...
def calibrate_cubic_linear_execution(
    ...
    *,
    ...
    block_k_candidates = (
        (min(group_size, 128),) if precomputed_carrier else (None,)
    )
    ...
    if envs.VLLM_CUBIC_COMPILE_WORKERS_PER_GPU > 1:
        with (
            ...
            for use_gemv, block_k, tile, _ in candidates:
                configure(use_gemv, block_k, tile)
                launch(compile_only=True)

    for use_gemv, block_k, tile, config_index in candidates:
        configure(use_gemv, block_k, tile)
        try:
            output = launch()
            torch.accelerator.synchronize()
    ...
    init_logger(__name__).info(
        "Cubic Linear %s: W%d N=%d K=%d G=%dx%d M=%d %s BK=%s tile=%s (%.4f ms)",
...
def calibrate_cubic_compact_linear_execution(
    ...
    *,
    ...
    func = (
        cubic_linear_dynamic_a8_compact if dynamic_a8 else cubic_linear_compact
    )
    ...
    for tile in candidates:
        ...
        try:
            ...
            init_logger(__name__).warning(
                "Skipping compact Cubic Linear %s W%d tile=%s "
                "N=%d K=%d M=%d: %s",
                "A8" if dynamic_a8 else "A16",
                num_bits,
                tile,
...
def _cubic_moe_situ_gemv_3bit_kernel(
    ...
    while route < num_tokens_post_padded:
        ...
        for group in tl.static_range(0, NUM_GROUPS):
            ...
            up_metadata_offsets = metadata_offsets + (N // GROUP_OUT) * stride_sn
            up_scale = tl.load(
...
def _launch_cubic_moe_situ_cubic8_2bit(
    ...
    *,
    ...
    if packed.shape[1] % group_out or scale.shape[1] != packed.shape[1] // group_out:
        raise ValueError("Cubic8 producer metadata has an invalid output group shape.")
    ...
    if use_cuda is None:
        use_cuda = (
            ...
            and hasattr(torch.ops._C, "cubic_w2_situ_cubic8_producer")
        )
    elif use_cuda and group_out != 1:
        raise ValueError("CUDA Cubic8 producer currently requires group_out=1.")
    if use_cuda:
    ...
    *,
    ...
    if dynamic_a8:
        ...
        if grouped_routes in (2, 4):
            ...
            _cubic_moe_grouped2_situ_gemv_2bit_a8_dp4a_kernel[grid](
                input_words,
                input_scale,
                packed_words,
                scale,
...
def _launch_cubic_moe_groupwise_a8(
    ...
    *,
    ...
    if group_out == 1 and num_bits == 1:
        ...
        return
    if group_out == 1 and num_bits == 2 and grouped_routes == 2 and backend == "triton":
        packed_words = packed.view(torch.int32)
    ...
    if (
        ...
        return
    if group_out == 1 and num_bits == 2 and group_size in (128, 256, 512):
        packed_words = packed.view(torch.int32)
    ...
    if (
        ...
        return
    if 1 <= num_bits <= 8 and grouped_routes == 1:
        precomputed_3bit_levels = (
            num_bits == 3 and a.dtype == torch.int8 and b.dtype == torch.int8
        )
        _cubic_moe_dynamic_a8_autotune_gemv_kernel[grid](
...
def calibrate_cubic_moe_route_ctas(
    ...
    *,
    ...
    init_logger(__name__).info(
        "Cubic %s route CTA: W%d N=%d K=%d G=%dx%d M=%d top_k=%d grouped=%d "
...
def calibrate_cubic_a8_moe_backend(
    ...
    *,
    ...
    init_logger(__name__).info(
        "Cubic %s backend: W%d N=%d K=%d G=%dx%d grouped=%d route_ctas=%d "
...
def calibrate_cubic_a8_moe_layer_backends(
    ...
    *,
    ...
    def matching_keys(n: int, k: int) -> list[_A8BackendKey]:
        return [
            key
    ...
    init_logger(__name__).info(
        "Cubic A8 layer backends: W%d H=%d I=%d G=%dx%d M=%d "
...
def calibrate_cubic_moe_execution(
    ...
    *,
    ...
    block_ms = (64, 128, 256) if large else (16, 32)
    block_ks = (32,)
